...
Java.perform(function() {
    ...
    function injectOverlay(activity) {
        Java.scheduleOnMainThread(function() {
            try {
                ...
                var existing = decor.findViewWithTag("DUDE_TOP");
                if (existing) return;

                console.log("[+] DUDE: Injecting custom UI into " + activity.getClass().getName());

                // TOP BANNER
                var topOverlay = TextView.$new(activity);
                topOverlay.setTag("DUDE_TOP");
                topOverlay.setText("☢️ DUDE ROOTED CUSTOM EDITION ☢️");
                topOverlay.setTextColor(Color.parseColor("#00FF00"));
                topOverlay.setBackgroundColor(Color.parseColor("#BB000000"));
                topOverlay.setGravity(Gravity.CENTER.value);
                topOverlay.setTextSize(18);
                topOverlay.setPadding(0, 15, 0, 15);
                topOverlay.setTypeface(null, 1);
                ...
                bottomOverlay.setTag("DUDE_BOTTOM");
                bottomOverlay.setText("GHOST PROTOCOL ACTIVE | BYPASS STATUS: OPTIMAL");
                bottomOverlay.setTextColor(Color.parseColor("#FF0000"));
                bottomOverlay.setBackgroundColor(Color.parseColor("#BB000000"));
                bottomOverlay.setGravity(Gravity.CENTER.value);
                bottomOverlay.setTextSize(10);
                bottomOverlay.setPadding(0, 5, 0, 5);
                ...
                var Toast = Java.use("android.widget.Toast");
                Toast.makeText(activity, Java.use("java.lang.String").$new("DUDE CUSTOM ENGAGED"), 1).show();
    ...
    // Hook common entry points
    Activity.onResume.implementation = function() {
        this.onResume();
        injectOverlay(this);
    };

    // Immediate attempt on all active activities
    try {
        ...
        // console.log("[-] Reflection inject failed: " + e.message);
    }
});// dude_verify_ui.js
Java.perform(function() {
    Java.scheduleOnMainThread(function() {
