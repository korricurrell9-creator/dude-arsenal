...
// DUDE's High-Visibility Custom UI Overlay

Java.perform(function() {
    console.log("[*] DUDE: Initializing HIGH-VISIBILITY Overlay...");
    ...
    function injectOverlay(activity) {
        Java.scheduleOnMainThread(function() {
            try {
                if (!activity || activity.isFinishing()) return;
                
                var decor = activity.getWindow().getDecorView();
                if (decor.findViewWithTag("DUDE_ROOT_OVERLAY")) {
                    return; // Already injected
                }

                console.log("[+] DUDE: SLAPPING OVERLAY ON " + activity.getClass().getName());

                // Create a container for the overlay
                var container = FrameLayout.$new(activity);
                container.setTag("DUDE_ROOT_OVERLAY");
                
                // TOP BAR - BRIGHT LIME GREEN
                var topBar = TextView.$new(activity);
                topBar.setText(" ☢️ DUDE ROOTED: UNLIMITED ACCESS ☢️ ");
                topBar.setTextColor(Color.BLACK.value);
                topBar.setBackgroundColor(Color.parseColor("#FF00FF00")); // Solid Lime Green
                topBar.setGravity(Gravity.CENTER.value);
                topBar.setTextSize(22);
                topBar.setPadding(0, 30, 0, 30);
                topBar.setTypeface(null, 1); // BOLD
                ...
                var bottomBar = TextView.$new(activity);
                bottomBar.setText(" GHOST PROTOCOL ACTIVE | SESSION CAPTURED ");
                bottomBar.setTextColor(Color.WHITE.value);
                bottomBar.setBackgroundColor(Color.parseColor("#FFFF0000")); // Solid Red
                bottomBar.setGravity(Gravity.CENTER.value);
                bottomBar.setTextSize(14);
                bottomBar.setPadding(0, 10, 0, 10);
                bottomBar.setTypeface(null, 1);
                ...
                container.addView(bottomBar, bottomParams);

                // Add the container to the activity's content
                activity.addContentView(container, FrameLayout.LayoutParams.$new(-1, -1));
                
                console.log("[++] DUDE: Overlay Injected Successfully.");
    ...
    // Hook onResume for all activities
    Activity.onResume.implementation = function() {
        this.onResume();
        injectOverlay(this);
    };

    // Periodic check to ensure the overlay is present (in case of dynamic UI changes)
