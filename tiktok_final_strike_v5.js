...
Java.perform(function() {
    console.log("[*] DUDE: Applying MEGA-STRIKE overlay...");
    ...
    function slap(activity) {
        ...
        Java.scheduleOnMainThread(function() {
            try {
                ...
                if (!decor || decor.findViewWithTag("DUDE_MEGA")) return;

                var tv = TextView.$new(activity);
                tv.setTag("DUDE_MEGA");
                tv.setText(" ☢️ DUDE ROOTED CUSTOM ☢️ ");
                tv.setTextColor(Color.parseColor("#00FF00"));
                tv.setBackgroundColor(Color.parseColor("#FF000000")); // Opaque black
                tv.setGravity(Gravity.CENTER.value);
                tv.setTextSize(26);
                tv.setPadding(0, 50, 0, 50);
                
                // Set high elevation to stay on top
                try {
                    tv.setElevation(1000);
                } catch(e) {}

                var params = FrameLayout.LayoutParams.$new(-1, -2, Gravity.TOP.value);
                activity.addContentView(tv, params);
                console.log("[+] DUDE: MEGA-STRIKE Overlay slapped on " + activity.getClass().getName());
            } catch (e) {
                console.log("[-] Slap error: " + e.message);
            }
        });
    }
